...
export default function CommandPalette() {
  ...
  return (
    <div className="fixed inset-0 z-[80] flex items-start justify-center pt-[14vh] px-4" role="dialog" aria-modal="true" aria-label="Buscador de secciones">
      <div className="absolute inset-0 bg-[#2C2118]/55 backdrop-blur-sm" onClick={close} />
      <div className="palette-in relative w-full max-w-lg rounded-3xl bg-[#FFFCF5] dark:bg-slate-900 ring-1 ring-[#E5D9C9] dark:ring-slate-700 shadow-2xl overflow-hidden">
        <div className="flex items-center gap-3 px-4 border-b border-[#E5D9C9] dark:border-slate-800">
              ...
              <li key={it.to}>
                <button
                  ...
                  className={`w-full flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-semibold text-left transition-colors ${
                    i === active
                      ? 'bg-[#A85638]/10 text-[#A85638] dark:bg-[#A85638]/20 dark:text-[#F6EEE3]'
        ...
        <div className="px-4 py-2.5 border-t border-[#E5D9C9] dark:border-slate-800 flex items-center gap-3 text-[11px] text-slate-400">
          <span><kbd>↑</kbd> <kbd>↓</kbd> navegar</span>
          <span><kbd>↵</kbd> abrir</span>
          <span className="ml-auto font-display italic text-[#C99A3C]">Santa Fe</span>
        </div>
      </div>
    </div>
  )
