...
function NuevoPedido() {
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-[1fr_360px] min-h-screen bg-[#FDF6EC]">

      {/* ── MENÚ ── */}
      <section className="p-4 lg:p-6 overflow-y-auto">
        {/* Header sticky */}
        <div className="sticky top-0 z-10 bg-[#FDF6EC]/95 backdrop-blur-sm pb-3 mb-4 border-b border-[#e8e0d8]">
          ...
          <input
            ...
            className="w-full rounded-xl border border-[#e8e0d8] bg-white px-4 py-3 text-sm outline-none focus:border-[#C1440E] focus:ring-2 focus:ring-[#C1440E]/10 transition-all"
          />
              ...
              return (
                <article
                  ...
                  className={`bg-white rounded-2xl shadow-sm overflow-hidden ring-1 transition-all ${
                    qty > 0 ? 'ring-[#C1440E] ring-2' : 'ring-slate-200'
                  }`}
                >
                  <div className="relative">
                    <img src={plato.imagen} alt={plato.nombre} className="w-full h-32 object-cover" />
                    {qty > 0 && (
                      <span className="absolute top-2 right-2 w-6 h-6 rounded-full bg-[#C1440E] text-white text-xs font-bold flex items-center justify-center shadow">
                        {qty}
                      </span>
                    )}
                  </div>
                  <div className="p-3">
                    <div className="flex items-start justify-between gap-2 mb-1">
                      ...
                      <span className="font-bold text-[#C1440E] text-sm whitespace-nowrap">S/ {plato.precio.toFixed(2)}</span>
                    </div>
                    <div className="mt-2">
                      {qty === 0 ? (
                        <button
                          onClick={() => agregarItem(plato)}
                          className="w-full rounded-lg bg-[#C1440E] text-white py-2 text-sm font-semibold hover:bg-[#a33a0c] active:scale-95 transition-all"
                        >
                          + Añadir
                        </button>
                      ) : (
                        <div className="flex items-center justify-between rounded-lg bg-[#C1440E]/10 p-1">
                          <button
                          ...
                          <span className="font-bold text-[#C1440E]">{qty}</span>
                          <button
                            onClick={() => cambiarCantidad(plato.nombre, +1)}
                            className="size-9 rounded-md bg-white shadow-sm text-slate-700 font-bold hover:bg-slate-50 transition-colors"
                          >
      ...
      {/* ── CARRITO ── */}
      <aside className="border-l border-[#e8e0d8] bg-white p-4 lg:p-5 lg:sticky lg:top-0 lg:h-screen flex flex-col gap-4">
        ...
        <div>
          ...
          <select
            ...
            className="w-full p-2.5 border border-slate-200 rounded-xl text-sm bg-white focus:outline-none focus:border-[#C1440E] focus:ring-2 focus:ring-[#C1440E]/10 transition-all"
          >
            ...
            <ul className="space-y-2">
              {itemsPedido.map(item => (
                <li key={item.nombre} className="flex items-center gap-2 text-sm bg-slate-50 rounded-xl px-3 py-2">
                  ...
                  >−</button>
                  <span className="w-5 text-center font-bold text-[#C1440E]">{item.cantidad}</span>
                  <button
                    onClick={() => cambiarCantidad(item.nombre, +1)}
                    className="size-7 rounded-lg bg-white shadow-sm text-slate-500 hover:text-[#C1440E] font-bold transition-colors"
                  >+</button>
          ...
          <button
            ...
            className={`w-full rounded-xl py-3 font-bold text-sm transition-all ${
              confirmado
                ...
                : canEnviar
                  ? 'bg-[#6B7C4F] text-white hover:bg-[#5a6842] active:scale-95 shadow-md shadow-[#6B7C4F]/20'
