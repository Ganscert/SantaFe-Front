    ...
    <div className="min-h-screen">
        ...
        <div className="max-w-5xl mx-auto px-4 py-5 pl-16 lg:pl-4">
          ...
          <div className="relative mb-3">
            ...
            <input
              ...
              style={{ fontSize: '16px' }}
              className="w-full pl-10 pr-3 py-2.5 rounded-xl border border-[#E2E8F0] dark:border-slate-700 bg-white dark:bg-slate-950 text-sm text-slate-700 dark:text-slate-200 placeholder:text-slate-400 outline-none focus:border-[#4F46E5] focus:ring-2 focus:ring-[#4F46E5]/10 transition-all"
            />
          </div>
          ...
          <div className="flex gap-2 overflow-x-auto pb-1">
            {CATEGORIAS.map((c) => (
              ...
              >
                {c === 'Favoritos' && <Star size={12} className={categoria === c ? 'fill-white' : 'fill-[#0EA5E9] text-[#0EA5E9]'} />}
                {c}
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
          {filtrados.map((plato) => (
            <article
              ...
              className={`cursor-pointer bg-white dark:bg-slate-900 rounded-3xl overflow-hidden shadow-sm ring-1 ring-[#E2E8F0] dark:ring-slate-800 hover:-translate-y-0.5 hover:shadow-md focus:outline-none focus:ring-2 focus:ring-[#4F46E5] transition-all ${plato.disponible === false ? 'opacity-75' : ''}`}
            >
              <div className="relative w-full h-44 bg-[#EEF2FF] dark:bg-slate-800 flex items-center justify-center">
                {plato.imagen ? (
                ...
                {esPlatoNuevo(plato) && (
                  <span className="absolute top-2 right-2 text-[10px] font-bold uppercase tracking-widest px-2 py-0.5 rounded-full bg-[#0EA5E9] text-white shadow-sm">
                    Nuevo
                  </span>
                )}
                <button
                ...
                >
                  <Star
                    size={15}
                    className={favoritos.has(plato.nombre) ? 'fill-[#0EA5E9] text-[#0EA5E9]' : 'text-slate-400'}
                  />
                </button>
              ...
              <div className="p-4">
                <div className="flex items-start justify-between gap-2 mb-2">
                  <div>
                    ...
                    <span className="text-xs text-[#10B981] dark:text-[#6EE7B7] font-semibold">{plato.categoria}</span>
                  </div>
                  <span className="text-[#4F46E5] dark:text-[#0EA5E9] font-black text-lg whitespace-nowrap">
                ...
                <ul className="mt-2 flex flex-wrap gap-1">
                  {plato.ingredientes.slice(0, 4).map((ing, j) => (
                    <li
                      key={j}
                      className="text-xs bg-[#EEF2FF] dark:bg-slate-800 text-slate-600 dark:text-slate-300 px-2 py-0.5 rounded-full border border-[#E2E8F0] dark:border-slate-700"
                    >
                      {ing}
                    </li>
                  ))}
      ...
      {seleccionado && (
        ...
        >
          ...
          <div className="relative w-full max-w-lg bg-white dark:bg-slate-900 rounded-3xl shadow-2xl ring-1 ring-[#E2E8F0] dark:ring-slate-800 overflow-hidden max-h-[90vh] flex flex-col">
            <div className="relative h-56 bg-[#EEF2FF] dark:bg-slate-800 flex items-center justify-center shrink-0">
              {seleccionado.imagen ? (
              ...
              <span className="absolute top-3 left-3 text-[10px] font-bold uppercase tracking-widest px-2.5 py-1 rounded-full bg-white/95 dark:bg-slate-900/95 text-[#10B981] dark:text-[#6EE7B7] border border-[#E2E8F0] dark:border-slate-700">
            ...
            <div className="p-5 overflow-y-auto">
              <div className="flex items-start justify-between gap-3 mb-3">
                ...
                <span className="text-[#4F46E5] dark:text-[#0EA5E9] font-black text-2xl whitespace-nowrap">
              ...
              <ul className="flex flex-wrap gap-1.5">
                {seleccionado.ingredientes.map((ing, i) => (
                  <li
                    key={i}
                    className="text-sm bg-[#EEF2FF] dark:bg-slate-800 text-slate-700 dark:text-slate-200 px-3 py-1 rounded-full border border-[#E2E8F0] dark:border-slate-700"
                  >
                    {ing}
                  </li>
                ))}
            ...
            <div className="px-5 pb-5 pt-2 border-t border-[#E2E8F0] dark:border-slate-800 flex justify-end shrink-0">
              <button
                type="button"
                onClick={() => setSeleccionado(null)}
                className="px-5 py-2.5 rounded-xl bg-[#4F46E5] text-white text-sm font-bold hover:bg-[#4338CA] transition-colors"
              >
                Cerrar
              </button>
            </div>
